...
| Прогон | Что |
|---|---|
| `npm run check:offline` | 53/53 — честный статус офлайн-устройства, перечитка конфига без рестарта, отказ при дублях id, выживание без `sdb`; парсер CPU-профиля на фикстурах обоих форматов (совпадающие числа, спец-узлы отдельно, рекурсия не удваивается) и деминификация топа с деградацией до `warning` |
...
| `npm run check:browser` | 44/44 в Chrome — capabilities, отказ `tv_install`, свой Chrome на порту 0, навигация, реальный скриншот, кейс лонгтапа в trusted и synthetic, CPU-профиль (именованная busy-функция видна в топе, двойной `start` и сиротский `stop` отвергнуты, профилирование шагами сценария), уборка за собой |
