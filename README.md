...
| `tv_record` | Записать путь **физическим пультом** и скомпилировать в готовый `tv_sequence` + чек-лист. На экране горит «● REC» |
...
```
tv_record {"action": "start"}                      # на экране загорается «● REC»
   … человек ходит пультом …
tv_record {"action": "status"}                     # сколько клавиш реально дошло до страницы
tv_record {"action": "stop", "title": "Лонгтап на плитке"}
```

`stop` возвращает `steps` **инлайном** — это то, что сразу скармливается в `tv_sequence`, и это 8–20 объектов; markdown при этом ложится на диск.
...
**Коллизия имён — вопрос человеку, а не решение за него.** Файл существует → `stop` не пишет и молча не суффиксует: возвращает `{"written": false, "conflict": "<path>"}` вместе со steps, а скомпилированный кейс держится в сессии. Дальше — `action: "write"` с явным `path` либо `overwrite: true`.
